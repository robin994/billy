...
def test_frontend_and_manifest_use_rewrite_version():
    ...
    assert "./bill-tracker-i18n.js?v=0.11.10-r2" in implementation
    assert '"version": "0.11.10"' in manifest
    assert 'FRONTEND_VERSION = "0.11.10"' in const
    assert 'FRONTEND_CACHE_VERSION = "0.11.10-r2"' in const
...
def test_billy_sidebar_panel_keeps_card_and_parser_manager():
    ...
    for token in (
        "billy-parser-manager.js?v=0.11.10-r2",
...
def test_new_billy_frontends_support_all_shipped_languages():
    ...
    assert "billy-extra-i18n.js?v=0.11.10-r2" in panel
    assert "billy-extra-i18n.js?v=0.11.10-r2" in parser
